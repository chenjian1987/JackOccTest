...
void HomePageActionFun::BsplineCurvePointEvalCompareTest()
{
    ...
    m_outputFunc(QStringLiteral("============================================================"));
    m_outputFunc(QStringLiteral("B样条 D0 点值求解 / De Boor 对比验证开始"));
    ...
    m_outputFunc(QStringLiteral("4) 青色 / 紫色 = De Boor 递推中间层"));
    m_outputFunc(QStringLiteral("5) 黄色点 = 最终点（同时对应 OCC::D0 与 De Boor 结果）"));
    m_outputFunc(QStringLiteral("============================================================"));
    ...
    m_outputFunc(QStringLiteral("[PointEvalCase] 构造结果：%1").arg(buildMsg));
    ...
    m_outputFunc(BSplineValidationUtils::BuildCurveSummary(QStringLiteral("PointEvalCase-多段三次B样条"), curve));

    const double u = 2.35;
    BSplinePointEvaluationResult evalResult;
    if (!BSplineValidationUtils::EvaluatePointByDeBoor(curve, u, evalResult))
    {
        m_outputFunc(QStringLiteral("[PointEvalCase] 点值验证失败：%1").arg(evalResult.message));
        return;
    }
    {
    ...
    BSplineValidationUtils::LogPointEvaluationSamplingComparison(   m_outputFunc,  QStringLiteral("PointEvalCase-全参数域"),  curve,  80);

    m_outputFunc(QStringLiteral("观察结论：当前参数 u = 2.35 只激活 4 个局部控制点。"));
    m_outputFunc(QStringLiteral("De Boor 递推得到的黄色点与 OCC::D0 的结果重合，说明 D0 背后遵循的是明确的 B 样条点值求解逻辑。"));
    m_outputFunc(QStringLiteral("============================================================"));
}
