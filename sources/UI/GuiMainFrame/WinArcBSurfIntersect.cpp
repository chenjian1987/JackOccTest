...
WinArcBSurfIntersect::WinArcBSurfIntersect(Handle(AIS_InteractiveContext) context, Handle(V3d_Viewer) v3dViewer, Handle(V3d_View) v3dView, QWidget* parent, std::function<void(const QString&)> func)
    : QDialog(parent), m_context(context), m_v3dViewer(v3dViewer), m_v3dView(v3dView), m_outputFunc(func)
{
    setupUI();
    setWindowTitle(QString::fromLocal8Bit("圆弧 × B样条面相交测试"));
    setMinimumSize(720, 600);

    m_util = NEW_AS_OWNER_PTR(ArcBSplineIntersect);
}

QWidget* WinArcBSurfIntersect::makeXYZRow(const QString& title,
...
QWidget* WinArcBSurfIntersect::makeCtrlRow(const QString& name,
    QLineEdit*& ex, QLineEdit*& ey, QLineEdit*& ez,
    const gp_Pnt& P)
{
...
void WinArcBSurfIntersect::setupUI()
{
    // 圆弧输入
    ...
    m_comboMethod->addItems({
        QString::fromLocal8Bit("IntCS"),
        QString::fromLocal8Bit("Section"),
        QString::fromLocal8Bit("EdgeFace"),
        QString::fromLocal8Bit("Extrema")
        });
    ...
    connect(m_btnCompute, &QPushButton::clicked, this, &WinArcBSurfIntersect::onCompute);
    connect(m_btnClear, &QPushButton::clicked, this, &WinArcBSurfIntersect::onClear);
...
void WinArcBSurfIntersect::drawArc(const gp_Pnt& center, double radius, double startAngle, double endAngle)
{
...
void WinArcBSurfIntersect::drawFace(const Handle(Geom_BSplineSurface)& surf)
{
...
void WinArcBSurfIntersect::drawPoint(const gp_Pnt& p, Quantity_NameOfColor color, const QString& name)
{
...
void WinArcBSurfIntersect::onClear()
{
...
void WinArcBSurfIntersect::onCompute()
{
    onClear();

    // 读取圆弧输入
    bool okR, okS, okE, okx, oky, okz;
    ...
    if (!(okR && okS && okE && okx && oky && okz) || R <= 0) {
        QMessageBox::warning(this, QString::fromLocal8Bit("输入错误"), QString::fromLocal8Bit("圆弧参数非法"));
        return;
    }
    ...
    // 显示几何
    drawFace(surf);
    drawArc(center, R, S, E);

    // 参数与方法
    ArcBSplineIsectParams prm;
    prm.tol3d = m_spinTol3d->value();
    prm.tolNear = m_spinTolNear->value();
    prm.discretSamples = m_spinSamples->value();
    prm.refineOnSurface = m_chkRefine->isChecked();

    EnumArcBsplineMethod method = EnumArcBsplineMethod::IntCS;
    switch (m_comboMethod->currentIndex()) {
    case 0: method = EnumArcBsplineMethod::IntCS;      break;
    case 1: method = EnumArcBsplineMethod::Section;    break;
    case 2: method = EnumArcBsplineMethod::EdgeFace;   break;
    case 3: method = EnumArcBsplineMethod::Extrema;    break;
    }

    // 求交
    std::vector<gp_Pnt> outPts;
    bool ok = m_util->IntersectArcWithBSplineFace(arcEdge, face, outPts, method, prm);

    // 标注交点
    int idx = 1;
    for (const auto& p : outPts)
        drawPoint(p, Quantity_NOC_GREEN, QString("IP%1").arg(idx++));

    // 信息
    QString info = QString::fromLocal8Bit("%1：%2 个交点   tol3d=%3   tolNear=%4   samples=%5")
        .arg(m_comboMethod->currentText())
        .arg(outPts.size())
        .arg(prm.tol3d, 0, 'g', 6)
        .arg(prm.tolNear, 0, 'g', 6)
        .arg(prm.discretSamples);
